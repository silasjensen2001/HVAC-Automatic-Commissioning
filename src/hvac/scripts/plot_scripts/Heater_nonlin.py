...
# ── Initial conditions ────────────────────────────────────────────────────────
T_init     = np.full(K, 9.9 + 273.15)   # air   [K]
theta_init = np.full(K,  9.9 + 273.15)   # water [K]
...
sol = solve_ivp(
    ode, (0, t_end), x0,
    t_eval=t_eval,
    method="Radau", # stiff solver instead of default RK45
    rtol=1e-6,
    atol=1e-8,
)
...
# export plot
#fig.savefig("heater_simulation.png", dpi=400)
